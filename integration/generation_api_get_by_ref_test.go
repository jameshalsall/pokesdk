//go:build integration

package integration

import (
	"context"
	_ "embed"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"

	"github.com/jameshalsall/pokesdk"
)

func TestGenerationGetByRef(t *testing.T) {
	server := NewMockServer(t)
	defer server.Close()

	ctx := context.Background()

	server.StubGET("/generation/1", Response{
		StatusCode: 200,
		Body:       generationResponse,
	})

	t.Run("it returns a specific generation by ref", func(t *testing.T) {
		client := server.PokeSDKClient()

		ref := pokesdk.GenerationRef{
			Name: "generation-i",
			URL:  server.URL() + "/generation/1",
		}
		generation, err := client.Generation.GetByRef(ctx, ref)
		require.NoError(t, err)

		assert.NotNil(t, generation)
		assert.Equal(t, "generation-i", generation.Name)
		assert.Equal(t, 1, generation.ID)

		reqs := server.Requests()
		require.Len(t, reqs, 1)
		assert.Equal(t, "/generation/1", reqs[0].Path)
	})

	t.Run("it returns an error if generation not found", func(t *testing.T) {
		server.Reset()
		client := server.PokeSDKClient()

		ref := pokesdk.GenerationRef{
			Name: "nonexistent",
			URL:  server.URL() + "/generation/99",
		}
		_, err := client.Generation.GetByRef(ctx, ref)
		require.Error(t, err)
		assert.ErrorIs(t, err, pokesdk.ErrGenerationNotFound)

		reqs := server.Requests()
		require.Len(t, reqs, 1)
		assert.Equal(t, "/generation/99", reqs[0].Path)
	})
}
